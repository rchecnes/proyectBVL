...
function savDepositoPlazo($link, $data, $dp_emp_id){
    ...
    foreach ($data as $d) {
        ...
        $sql .= "('$dp_emp_id','$dh_pono','$dh_fecha','$dh_sal_prom_d','$dh_sal_prom_h','$dh_plazo_d','$dh_plazo_h','$dh_tea','$dh_last_update','1','$dh_time'),";
    ...
    if ($sql !='') {
        ...
        $delete = "DELETE FROM historico_deposito_plazo WHERE dh_emp_id='$dp_emp_id' AND dh_fecha='".date('Y-m-d')."'";
        $respdel = mysqli_query($link, $delete);

        $insert = "INSERT INTO historico_deposito_plazo (dh_emp_id, dh_pono, dh_fecha, dh_sal_prom_d, dh_sal_prom_h, dh_plazo_d, dh_plazo_h, dh_tea, dh_last_update, dh_stat, dh_time) VALUES ".trim($sql,',').";";
...
function getDepositoPlazo(){
	...
	while ($e = mysqli_fetch_array($respemp)) {
        ...
        $url = "https://comparabien.com.pe/producto/depositos-plazo/fondesurco-plazo-fijo-mn?prod_id=160&type=DEPOSITOS";
        ...
        if (count($data_sav)>0) {
            ...
            $res = savDepositoPlazo($link, $data_sav, $e['dp_emp_id']);
